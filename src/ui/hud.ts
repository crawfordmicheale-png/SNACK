...
import { Thornmaw, Tideheart, Cindermouth } from '../game/entities/boss';
...
function drawBossBar(u: UiCtx, scene: Scene): void {
  const boss =
    ...
    scene.entities.find((e): e is Cindermouth => e instanceof Cindermouth && !e.dead && e.isAwake);
  if (!boss) return;
  const w = u.w * 0.6;
  const x = (u.w - w) / 2;
  const y = 12;
  const name =
    boss instanceof Cindermouth ? 'CINDERMOUTH' : boss instanceof Tideheart ? 'TIDEHEART' : 'THORNMAW';
